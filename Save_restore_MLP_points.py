...
#Network parameters need to change  
num_points=25
...
model_file_base='/ifs/loni/faculty/thompson/four_d/Faisal/Projects/NeuralNet_TBI/Outputs/' + subj + '/Trained_models_corrected/' + str(num_points) + '_points/Inverse_tracks/2_layers/' + str(n_hidden_1) + '_' + str(n_hidden_2) + '_' + 'Batch_size_' + str(batch_size) + '/'
...
with tf.Session() as sess:
    ...
    for epoch in range(start, training_epochs):
        ...
        total_batch = int(num_train_set/batch_size)
        # Loop over all batches
        for i in range(total_batch):
            # batch_x, batch_y = mnist.train.next_batch(batch_size)
            testClass = gb.GenerateBatches()
            batch_xs, batch_ys = testClass.CreateBatches_XY(num_train_set, train_name_list, batch_size, num_fiber_bundles, Track_label_lookup, num_points)
        ...
        if epoch % display_step == 0:
            global_step.assign(epoch+1).eval()
            model_file_path_i = model_file_base + 'MLP_epoch_' + str(epoch+1) + '.cpkt'
