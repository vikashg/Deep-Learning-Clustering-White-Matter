...
training_fileList_randomized=base_dir + 'training_set_randomised_new_wo_cc_frontal_' + str(num_points) + '.txt'
training_file_fiber_names = open(training_fileList_randomized, "r")
train_name_list = training_file_fiber_names.readlines()

testing_fileList_randomized=base_dir + 'testing_set_randomised_new_wo_cc_frontal_' + str(num_points)+ '.txt'
...
batch_size_train = num_train_set
testClass = gb.GenerateBatches()
batch_xs_train, batch_ys_train = testClass.CreateBatches_XY(num_train_set, train_name_list, batch_size_train, num_fiber_bundles, Track_label_lookup, num_points)
...
model.fit({'input': batch_X_train}, {'target': batch_ys_train}, n_epoch= extra_epoch,validation_set=({'input': batch_X_test}, {'target': batch_ys_test}), snapshot_step=100, show_metric=True, batch_size=3000, run_id='convnet_track_ver_3_conv_layers')

total_epoch = extra_epoch + num_epoch
model_name = output_dir + 'CNN_conv2d_32_conv2d_32_conv2d_32_fc_64_fc_128_epoch_' + str(total_epoch) + '.tflearn'
