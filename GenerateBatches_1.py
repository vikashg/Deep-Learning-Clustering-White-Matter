...
class GenerateBatches():
    ...
    def CreateBatches_XY(self, num_fiber_set, train_name_list, num_fibers_bundles  , Track_label_dictionary, num_points):
        ...
        batch_X = np.full((num_fiber_set, 3*num_points), 1.0, dtype = 'float')
        batch_Y= self.Create_batchY(num_fiber_set, train_name_list, num_fibers_bundles, Track_label_dictionary)

        ## Extract File Names
        for i in range(0, num_fiber_set ):
    ...
    def Create_batchY(self, num_fiber_set, fiber_names_list, num_fibers, Track_lookup_table):
